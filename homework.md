...
- Write logic to handle GET, POST, PATCH, PUT, DELETE etc.... and test in postman - // creating server using express
  const express = require("express");
...
- // pattern routing with regular expression - app.get("/ab?c",(req,res)=>{}) => I have used ? so now b became optional SO if I write /ac, /abc => both will work - // creating server using express
  const express = require("express");
...
- How to make dynamic routing - http://localhost:7777/user/102
...
- Explore schema types options from the documentations => https://mongoosejs.com/docs/schematypes.html
- add redquired, unique, min, max, minlength, maxlength, lowercase, trim,
- add default
...
- Logic for GET /feed API - most important
- Explore about query condition - $nin, $and, $ne and others query operators => https://www.mongodb.com/docs/manual/reference/operator/query/nin/
